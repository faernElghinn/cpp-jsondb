...
public:
    ...
    size_t count(const std::string& clas, json::Json_t id);
    size_t countConditionnal(const std::string& clas, const std::shared_ptr<jsondb::Clause> where);
    json::Json_t load(const std::string& clas, json::Json_t id);
    std::vector<json::Json_t> loadAll(std::string clas, Sort = Sort());
    std::vector<json::Json_t> loadConditionnal(const std::string& clas, const std::shared_ptr<Clause> where, Sort = Sort());
    void remove(const std::string& clas, json::Json_t id);
    void remove(const std::string& clas, const std::shared_ptr<jsondb::Clause> where);

    inline const std::string& getBasePath() const { return _basepath;}
    inline const std::map<std::string, std::map<std::string, Index> >& getIndex() const { return _indexes;}
...
protected:
    ...
    std::map<std::string, std::map<std::string, Index> > _indexes;
