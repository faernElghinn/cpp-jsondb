...
CmpClause::CmpClause(const std::string& path, Mode mode, Json_t value, bool caseSensitive) :
    ...
    if (_path == ID)
        _expected = std::make_shared<JsonString>(toString(value));

    if (_expected->getType() == JSON_STRING) {
        // Remove the "".
        std::string str = _expected->toString()->value.substr(1);
        str.pop_back();

        if (!_caseSensitive)
            str = toLower(str);

        _expected = std::make_shared<JsonString>(str);
    }
}
    ...
    if (_mode == NE) {
        for (auto ite : matchingElements) {
            ...
            if (ite->getType() != _expected->getType())
                throw Exception("Element " + _path + " of " + toString(root->getChild(root, ID).front()) + " is of the wrong type.");
...
bool CmpClause::testRelation(Json_t value) const {
    ...
    switch (_mode) {
        case NE:            return value != _expected;
        case EQ:            return value == _expected;
        case LT:            return value < _expected;
        case LE:            return value <= _expected;
        case GE:            return value >= _expected;
        case GT:            return value > _expected;
...
ExistClause::ExistClause(const std::string& path): Clause(), _path(path) {}
...
std::shared_ptr<Clause> Clause::And()   { return std::make_shared<AndClause  >(); }
std::shared_ptr<Clause> Clause::Or()    { return std::make_shared<OrClause   >(); }
